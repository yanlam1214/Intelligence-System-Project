...
def play_games_and_return_stats(engine: SchnapsenGamePlayEngine, bot1: Bot, bot2: Bot, number_of_games: int) -> int:
    ...
    for i in range(1, number_of_games + 1):
        ...
        winner, _, _ = engine.play_game(lead, follower, random.Random())#random.Random(i)
        if winner == bot1:
            bot1_wins += 1
        if i % 500 == 0:
...
def create_replay_memory_dataset() -> None:
    ...
    for i in range(1, num_of_games + 1):
        ...
        engine.play_game(replay_memory_recording_bot_1, replay_memory_recording_bot_2, random.Random())#random.Random(i))
...
def train_model() -> None:
    ...
    replay_memory_filename: str = 'rdeep_NN3.txt' # 'random_random_10k_games.txt'
...
def try_bot_game() -> None:
    ...
    model_dir: str = 'ML_models'
    model_name: str = 'simple_model'
    model_name2: str = 'random_NN1'
    model_location = pathlib.Path(model_dir) / model_name
    model_location2 = pathlib.Path(model_dir) / model_name2
    bot1: Bot = MLPlayingBot(model_location=model_location)
    bot2: Bot = MLPlayingBot(model_location=model_location2)
    # bot2: Bot = RandBot(464566)
    # bot2: Bot = RdeepBot(num_samples=16, depth=4, rand=random.Random(4564654644))
